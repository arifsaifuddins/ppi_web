...
function BlogPost({ data }) {
  ...
  return (
    <div className="flex flex-col p-4 rounded-xl w-full shadow-lg hover:shadow-xl bg-white dark:bg-transparent lg:hover:scale-[103%] transition-transform">
      ...
      <Link to={`/blog/${data.slug}`} ><h1 className="text-2xl text-teal-600 font-bold my-4 hover:text-teal-700 hover:underline">{data.title.length < 25 ? data.title : data.title.slice(0, 25) + '...'}</h1>
        <p className="font-light mb-4">{(body.replaceAll('&nbsp;', '').trim().length < 70) ? body.replaceAll('&nbsp;', '').trim() : body.replaceAll('&nbsp;', '').trim().slice(0, 70) + '...'}</p></Link>
