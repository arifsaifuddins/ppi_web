  ...
  return (
    <div className="p-4 flex lg:rounded-xl rounded-md gap-4 bg-slate-50 dark:bg-[#121212] shadow dark:shadow-black">
      ...
      <div className="flex flex-col justify-between">
        <div>
          <h1 className="font-bold text-teal-600">{data.title.slice(0, 25)}...</h1>
          <p className="text-xs mt-2"><i className="fa fa-user mr-1"></i> {data.author.slice(0, 15)}... <i className="fa mx-1 fa-calendar-days"></i> {data.year} </p>
        </div>
        <div className="flex gap-4 items-center">
          <p onClick={() => downloadFile(`${phpurl}/images/${data.pdf}`, data.pdf)} className="text-white bg-teal-600 hover:bg-teal-700 font-bold py-1 text-xs px-3 rounded-full cursor-pointer"><i className="fa fa-download mr-1"></i> Download</p>
          {
