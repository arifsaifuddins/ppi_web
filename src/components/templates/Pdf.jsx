  ...
  return (
    <div className="p-4 flex lg:rounded-xl rounded-md gap-4 bg-slate-50 dark:bg-[#121212] shadow dark:shadow-black">
      ...
      <div className="flex flex-col justify-between">
        <div>
          <h1 className="font-bold text-teal-600">{data.title.length < 20 ? data.title : data.title.slice(0, 20) + '...'}</h1>
          <p className="text-xs mt-1"><i className="fa fa-user mr-1"></i> {data.author.length < 10 ? data.author : data.author.slice(0, 10) + '...'} <i className="fa mx-1 fa-calendar-days"></i> {data.year} </p>
        </div>
