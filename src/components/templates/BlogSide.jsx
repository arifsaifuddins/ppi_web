...
function BlogSide({ data }) {
  ...
  return (
    <Link to={`/blog/${data.slug}`} className="flex p-4 gap-4 justify-between border-b hover:bg-slate-50 dark:hover:bg-slate-700">
      ...
      <img src={phpurl + '/images/' + data.blog_poster} alt="poster" className="h-20 w-32 object-cover rounded-xl" />
    </Link>
  );
}
