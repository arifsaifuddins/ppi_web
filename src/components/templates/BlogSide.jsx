...
function BlogSide({ data }) {
  ...
  return (
    <Link to={`/blog/${data.slug}`} className="flex p-4 gap-4 justify-between border-t hover:bg-slate-50 dark:hover:bg-[#333333]">
      <div className="flex justify-between flex-col">
        <p className="text-xl font-bold mb-2 text-teal-600 hover:underline">{data.title.length < 20 ? data.title : data.title.slice(0, 20) + '...'}</p>
